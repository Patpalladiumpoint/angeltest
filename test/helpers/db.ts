...
// deliberately separate from src/db/migrate.ts (which tracks state in
// schema_migrations for a long-lived database) because tests want a
// from-scratch, idempotent apply against a throwaway database every run.
...
// out of band in real deploys (see 0001_init.sql's comment). Tests do that
// step here, mirroring what a deploy script/Terraform would do once.
...
export function requireEnv(name: string): string {
  ...
  if (!value) {
    throw new Error(
      `${name} is not set. Integration tests need a real Postgres -- see README "Running tests locally".`,
    );
  }
  return value;
}
